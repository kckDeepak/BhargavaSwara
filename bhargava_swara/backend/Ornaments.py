...
def perform_analysis_threaded(audio_data, file_name):
    ...
    display_ornaments_output(analysis_result) # Corrected function call

def display_ornaments_output(analysis_result):
    output_text_area.config(state=tk.NORMAL)
    output_text_area.delete("1.0", tk.END)
    ...
    try:
        ...
        if start_index != -1 and end_index != -1:
            ornaments_part = analysis_result[start_index:end_index].strip()

            output_text_area.insert(tk.END, "-------------------\n")
            output_text_area.insert(tk.END, "EXTRACTED DETAILS\n\n")
            output_text_area.insert(tk.END, ornaments_part + "\n")
            output_text_area.insert(tk.END, "-------------\n")

        else:
            output_text_area.insert(tk.END, "Could not extract Ornaments information.\n")
