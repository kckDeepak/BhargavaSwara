...
def analyze_music_gemini(audio_data, file_name):
    """Analyzes music and returns analysis result for Ornaments only."""
    try:
...
def extract_ornamentation(analysis_result):
    """Extracts the ornamentation details from the Gemini response."""
    try:
        start_index = analysis_result.find("ORNAMENTS :-")
        end_index = analysis_result.find("-------------", start_index)
        if start_index != -1 and end_index != -1:
            ornaments_part = analysis_result[start_index + 13:end_index].strip()  # Skip "ORNAMENTS :- "
            return ornaments_part
        return "Unknown Ornamentation"
    except Exception:
        return "Error extracting Ornamentation"

@app.route('/analyze-ornamentation', methods=['POST'])
def analyze_ornamentation():
    ...
    detected_ornamentation = extract_ornamentation(analysis_result)

    # Clean up the uploaded file
    os.remove(file_path)

    return detected_ornamentation
